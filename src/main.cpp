...
#if defined(NDEBUG)
# error "Vagcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x300789ec97e21d002df38741273ecbb17febe1981b1625a377335ac82997a3c3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vagcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vagcoin Signed Message:\n";
...
{
    // Vagcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 900000); // Vagcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Vagcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Vagcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vagcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vagcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb3b6b8f96f98c399c7fc4e5e30a6386be612aecf4623960704c187ff9957ea09");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 4/21/2021 Vagcoin ibo eshak sonar 2pac";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044211ecca500f9301e5ad3296e08615211cefe32ae113a1ad039b51eba49160a7626760b3a26b609463199ddc50beef3b5ab88b28d82f494e6650b25f9aed7f35") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1619009068;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085172543;

        if (fTestNet)
        {
            block.nTime    = 1619009038;
            block.nNonce   = 385537609;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x28e1b3d017d571e4b4b83f486c0fe11b17f504e6cfc36522941e16a7d8a3698a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc4, 0xb2, 0xdf }; // Vagcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VagcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VagcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VagcoinMiner(CWallet *pwallet)
{
    printf("VagcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vagcoin-miner");
    ...
    try { loop {
        ...
        printf("Running VagcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VagcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VagcoinMiner, pwallet));
}
